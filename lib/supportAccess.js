...
export async function getUserByEmail(email) {
  const { data, error } = await supabase
    .from('users')
    .select(
      'email, role, access_status, oracle_credits_remaining, ' +
      'support_chat_credits_remaining, support_access_enabled, last_support_access_at'
    )
    .eq('email', email)
    .single();
...
export async function updateLastSupportAccess(email) {
  await supabase
    .from('users')
    .update({ last_support_access_at: new Date().toISOString() })
    .eq('email', email);
}
...
export function isApprovedForSupport(user, entitlements) {
  ...
  if (!adminOverride && !hasActiveProduct) return false;
...
export function buildAccountSnapshot(user, entitlements) {
  ...
  const oracleCredits   = user.oracle_credits_remaining ?? 0;
  const adminOverride   = user.support_access_enabled === true;
  ...
  return {
    ...
    last_support_access_at:         user.last_support_access_at || null,
  };
}
